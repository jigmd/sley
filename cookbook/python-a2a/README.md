...
This example exposes a Caskada research agent through the A2A JSON-RPC
protocol. The important integration point is `CaskadaTaskManager`:

1. Extract the question from the incoming A2A task.
2. Run the ordinary Caskada Flow.
...
Caskada workflow remains independent in `nodes.py` and `flow.py`.
