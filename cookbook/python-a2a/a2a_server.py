...
def main(host, port):
    """Starts the Sley A2A Agent server."""
    try:
        ...
        capabilities = AgentCapabilities(
            ...
            stateTransitionHistory=False # Sley state isn't exposed via A2A history
        )
        skill = AgentSkill(
            id="web_research_qa",
            ...
            inputModes=SleyTaskManager.SUPPORTED_CONTENT_TYPES,
            outputModes=SleyTaskManager.SUPPORTED_CONTENT_TYPES,
        )
        agent_card = AgentCard(
            name="Sley Research Agent (A2A Wrapped)",
            description="A simple research agent based on Sley, made accessible via A2A.",
            ...
            provider=None,
            authentication=None,
            defaultInputModes=SleyTaskManager.SUPPORTED_CONTENT_TYPES,
            defaultOutputModes=SleyTaskManager.SUPPORTED_CONTENT_TYPES,
        )

        # --- Initialize and Start Server ---
        task_manager = SleyTaskManager() # Instantiate your custom manager
        server = A2AServer(
        ...
        logger.info(f"Starting Sley A2A server on http://{host}:{port}")
        server.start()
